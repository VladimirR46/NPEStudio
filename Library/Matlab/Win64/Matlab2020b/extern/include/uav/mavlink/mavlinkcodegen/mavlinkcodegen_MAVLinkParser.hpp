...
class LIBMWMAVLINK_API MAVLinkParser : public MAVLinkParserInterface {
  public:
     ...
     */
    std::vector<mavlink_message_t> deserialize(uint8_t const* buffer, size_t len) override;
    ...
    /**
     * @param[in] entries   a 6*N row vector containing [msgid, msgck, maxlen, flags,
  ...
  private:
    /// Updates mapping between msgid and message entry index
    void updateMessageTbl();
     ...
     */
    MessageEntryRepository::MsgEntryLookupTbl m_table = {{0, {0, 50, 9, 0, 0, 0}},
                                                         {1, {1, 124, 31, 0, 0, 0}}};
};
} // namespace mavlink
} // namespace uavlib
