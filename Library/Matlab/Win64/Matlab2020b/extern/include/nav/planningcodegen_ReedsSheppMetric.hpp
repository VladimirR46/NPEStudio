...
class ReedsSheppMetric<real64_T> : public DistanceMetric<real64_T> {
  public:
    ReedsSheppMetric(real64_T turningRadius, real64_T reverseCost) {
        this->m_dim = 3;
    ...
    std::vector<real64_T> distance(const std::vector<real64_T>& states,
                                   const std::vector<real64_T>& queryState) {
...
// copying overhead. better way is to update the ttb functor to provide a row-major API.
        ...
        autonomousReedsSheppDistanceCodegen_real64(
            ...
            static_cast<uint32_T>(1), m_minTurningRadius, m_reverseCost, dists.data());
#else
